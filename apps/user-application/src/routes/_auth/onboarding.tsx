...
function OnboardingPage() {
	...
	const handleScanGitHub = async () => {
		...
		try {
			// Get user repositories (agent will initialize automatically)
			...
			if (!res.ok) {
				...
				console.error("Languages response:", errorText);
		...
		<div className="min-h-screen bg-gradient-to-br from-[#FBAD41] to-[#FF6633] flex items-center justify-center px-10">
			<div className="w-full h-full">
				<div className="text-center mb-8 text-secondary">
					<h1 className="text-5xl font-bold mb-2">Welcome to Contribot!</h1>
					<p className="">
						Let's personalize your experience. What languages do you work with,
						and how challenging would you like your first issues to be?
					</p>
				</div>
					...
					<div className="flex justify-center items-center relative ">
						<CardContent>
							<div className="flex justify-center flex-wrap gap-y-2 px-7">
								{Array.from(
									new Set([...COMMON_LANGUAGES, ...selectedLanguages]),
								)
									// .sort()
									.map((lang) => (
										<Button
					...
					<Card className="">
						...
						<CardContent className="space-y-3">
							...
							>
								...
								{scanningGitHub ? "Scanning..." : "Scan My GitHub"}
							</Button>
							<p className="text-sm text-muted-foreground">
								We'll analyze your repositories to detect languages you use
							</p>
						</CardContent>
						<CardHeader>
