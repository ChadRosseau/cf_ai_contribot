...
export function DashboardView() {
	...
	// Helper to reorder languages to show filtered languages first
	const prioritizeFilteredLanguages = (languages: string[]): string[] => {
		if (!issuesFilters?.languages || issuesFilters.languages.length === 0) {
			return languages;
		}

		// Split into filtered and non-filtered languages
		const filtered = languages.filter((lang) =>
			issuesFilters.languages?.includes(lang),
		);
		const notFiltered = languages.filter(
			(lang) => !issuesFilters.languages?.includes(lang),
		);

		// Return filtered first, then others
		...
		async (page: number) => {
			...
			try {
				...
				updateDashboardState({
					repos: data.repos || [],
					reposPage: page,
					reposTotalPages: Math.ceil((data.total || 0) / itemsPerPage),
				});
			} catch (error) {
	...
	const fetchRecommendedIssues = useCallback(
		async (
			...
			try {
				...
				setIssuesTotalPages(Math.ceil((data.total || 0) / itemsPerPage));

				// Sync to agent state (UI-initiated query)
				updateDashboardState({
					issues: data.issues || [],
					issuesPage: page,
					issuesTotalPages: Math.ceil((data.total || 0) / itemsPerPage),
	...
	useEffect(() => {
		...
		if (activeTab === "repos" && lastFetchedReposPage.current !== reposPage) {
			...
			// Fetch if: page changed OR filters changed
			// Always respect current filters when paginating
			const shouldFetch =
				lastFetchedIssuesPage.current !== issuesPage ||
				...
				<div className="flex-1 overflow-y-scroll p-4 space-y-4 max-h-[calc(100vh)] mt-3">
											...
											<div className="flex items-start justify-between">
												<div className="space-y-1 flex-1">
													...
													<p className="text-sm text-muted-foreground">
														...
														{repo.openIssuesCount === 1 ? "issue" : "issues"}
													</p>
												</div>
												<Button
													variant="ghost"
													size="sm"
													onClick={(e) =>
														toggleFavouriteRepoHandler(repo.id, e)
													}
													className="h-8 w-8 p-0"
												>
													<Star
														className={`h-4 w-4 ${
															favouriteRepos.has(repo.id)
								...
								{reposTotalPages > 1 && (
									<div className="flex items-center justify-center gap-2 py-4">
										...
										<Button
											...
											size="sm"
											onClick={() =>
												setReposPage(Math.min(reposTotalPages, reposPage + 1))
											}
											disabled={reposPage === reposTotalPages || loading}
											className="bg-white/90 backdrop-blur-sm"
										>
							...
							<Card className="bg-white/90 backdrop-blur-sm shadow-lg py-3">
								<CardContent className="space-y-2">
									...
									<div className="flex flex-wrap gap-2">
										{(issuesFilters?.repoFilter || selectedRepo) && (
										...
										{issuesFilters?.difficulty && (
											<div className="flex items-center gap-1 px-2 py-1 bg-purple-100 text-purple-800 rounded-md text-xs">
												<span>Difficulty: {issuesFilters.difficulty}/5</span>
												<Button
													variant="ghost"
													size="sm"
							...
							<>
								{issues.map((issue) => (
									...
									>
											...
											<div className="flex items-start justify-between">
												...
												<div className="flex items-center gap-2">
													...
													<Button
														...
														size="sm"
														onClick={(e) =>
															toggleFavouriteIssueHandler(issue.id, e)
														}
														className="h-8 w-8 p-0"
													>
														<Star
															className={`h-4 w-4 ${
																favouriteIssues.has(issue.id)
																	? "fill-yellow-400 text-yellow-400"
																	: "text-muted-foreground"
															}`}
														/>
										...
										<CardContent>
											<div className="text-sm text-muted-foreground mb-2 prose prose-sm max-w-none">
												<ReactMarkdown>{issue.intro}</ReactMarkdown>
											</div>
											<div className="flex gap-2 flex-wrap">
												{prioritizeFilteredLanguages(issue.languages)
													.slice(0, 3)
													.map((lang: string) => {
														const colors = getLanguageColor(lang);
														return (
								...
								{issuesTotalPages > 1 && (
									<div className="flex items-center justify-center gap-2 py-4">
										<Button
											variant="outline"
											size="sm"
											onClick={() => setIssuesPage(Math.max(1, issuesPage - 1))}
											disabled={issuesPage === 1 || loading}
											className="bg-white/90 backdrop-blur-sm"
										>
											<ChevronLeft className="h-4 w-4" />
										</Button>
										...
										<Button
											...
											onClick={() =>
												setIssuesPage(
													Math.min(issuesTotalPages, issuesPage + 1),
												)
											}
											disabled={issuesPage === issuesTotalPages || loading}
											className="bg-white/90 backdrop-blur-sm"
										>
											<ChevronRight className="h-4 w-4" />
										</Button>
									...
									<CardContent className="space-y-4">
										<div>
											...
											<div className="text-sm text-muted-foreground prose prose-sm max-w-none">
												<ReactMarkdown>{selectedIssue.intro}</ReactMarkdown>
											</div>
										</div>
					...
					<TabsContent value="favourites" className="mt-0 space-y-6">
						<div>
							...
							{repos.filter((r) => favouriteRepos.has(r.id)).length === 0 ? (
								<p className="text-sm text-white/70">
									No favourite repositories yet. Star repos to see them here.
								</p>
							) : (
						...
						<div>
							...
							{issues.filter((i) => favouriteIssues.has(i.id)).length === 0 ? (
								<p className="text-sm text-white/70">
									No favourite issues yet. Star issues to see them here.
								</p>
							) : (
								<div className="space-y-3">
									{issues
										.filter((i) => favouriteIssues.has(i.id))
										.map((issue) => (
											<Card
												key={issue.id}
												className="p-4 cursor-pointer hover:bg-accent transition-colors bg-white/90 backdrop-blur-sm shadow-lg"
