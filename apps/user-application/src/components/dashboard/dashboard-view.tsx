...
export function DashboardView() {
  const [activeTab, setActiveTab] = useState<"overview" | "list" | "issue" | "settings">("overview");
        ...
        <div className="flex-1 overflow-y-auto p-4">
          <TabsContent value="overview" className="mt-0 space-y-4">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 border rounded-lg">
                    ...
                    <p className="text-sm text-muted-foreground">Issues Started</p>
                  </div>
                  <div className="p-4 border rounded-lg">
                    <p className="text-2xl font-bold">0</p>
                    <p className="text-sm text-muted-foreground">PRs Submitted</p>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-2">
                <Button variant="outline" className="w-full justify-start" onClick={() => setActiveTab("list")}>
            ...
            {loading ? (
              ...
              <Card>
                <CardContent className="py-8 text-center">
                  <p className="text-muted-foreground">No issues found matching your preferences.</p>
              ...
              issues.map((issue) => (
                <Card key={issue.id} className="cursor-pointer hover:bg-accent transition-colors" onClick={() => handleSelectIssue(issue)}>
                  <CardHeader>
                  ...
                  <CardContent>
                    <p className="text-sm text-muted-foreground mb-2">{issue.intro}</p>
              ...
              <div className="space-y-4">
                ...
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {selectedIssue.owner}/{selectedIssue.repoName} #{selectedIssue.issueNumber}
                    </p>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                      <h4 className="font-semibold mb-2">About this issue</h4>
                      <p className="text-sm text-muted-foreground">{selectedIssue.intro}</p>
                    </div>

                    <div>
                      <h4 className="font-semibold mb-2">First Steps</h4>
                      <p className="text-sm text-muted-foreground">{selectedIssue.firstSteps}</p>
                    </div>
          ...
          <TabsContent value="settings" className="mt-0">
            <Card>
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  Settings panel coming soon. You can update your language preferences and difficulty level here.
                </p>
              </CardContent>
            </Card>
          </TabsContent>
