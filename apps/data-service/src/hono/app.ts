...
app.get("/test-scraper", async (c) => {
  try {
    ...
    if (!c.env.SCRAPER_WORKFLOW) {
      return c.json({
        success: false,
        error: "Workflow binding not available. Make sure you're running with 'wrangler dev' and workflows are configured in wrangler.jsonc",
...
app.get("/test-scraper-direct", async (c) => {
  try {
    ...
    const maxRepos = c.req.query("maxRepos") ? parseInt(c.req.query("maxRepos")!) : 6;
    const maxReposForIssues = c.req.query("maxReposForIssues") ? parseInt(c.req.query("maxReposForIssues")!) : 3;
    const maxIssuesPerRepo = c.req.query("maxIssuesPerRepo") ? parseInt(c.req.query("maxIssuesPerRepo")!) : 10;
...
app.get("/workflows", async (c) => {
  return c.json({
    success: true,
    message: "Use the Cloudflare dashboard or Workflows REST API to list all instances",
    endpoints: {
